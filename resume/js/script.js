((document) =>{
    ...
    let str = [
            ...
            //信息介绍页面
            '<p>  以下是个人基本信息</p>↓\\' +
            '<span>姓名: </span>\\<span class="green">戴垚</span>\\<div></div>\\' +
            '<span>出身: </span>\\<span class="green">1995.4.17</span>\\<div></div>\\' +
            '<span>性别: </span>\\<span class="green">男</span>\\<div></div>\\' +
            '<span>名族: </span>\\<span class="green">汉</span>\\<div></div>\\' +
            '<span>婚姻状况: </span>\\<span class="green">未婚</span>\\<div></div>\\' +
            '<span>政治面貌: </span>\\<span class="green">团员</span>\\<div></div>\\' +
            '<span>学历: </span>\\<span class="green">大专</span>\\<div></div>\\' +
            '<span>专业: </span>\\<span class="green">机械类专业(我是个自学者~)</span>\\<div></div>\\' +
            '<span>电话: </span>\\<span class="green">15258420950</span>\\<div></div>\\' +
            '<span>E-mail: </span>\\<span class="green">mrdai346139312@gmail.com</span>\\<div></div>\\' +
            '<span>github: </span>\\<span class="green">https://github.com/Mrdaigithub</span>\\<div></div>\\' +
            '<span>press enter to continue</span>\\',

            //技能介绍页面
            '<p class="tc">相关技能</p>↓\\' +
            '<span>1. </span>\\<span class="green">熟练掌握语义化的 HTML 和具有兼容性的 CSS 模式，熟练手写符合 W3C 标准的结构和代码。</span>\\<div></div>\\' +
            '<span>2. </span>\\<span class="green">对前端css，js性能优化有一定的了解</span>\\<div></div>\\' +
            '<span>3. </span>\\<span class="green">掌握sublime Text, Webstorm, photoshop等软件的使用</span>\\<div></div>\\' +
            '<span>4. </span>\\<span class="green">能使用Sass等css编译工具,gulp前端构建工具,并了解使用es6的一些新特新，如class,module,let,const,箭头函数等..</span>\\<div></div>\\' +
            '<span>5. </span>\\<span class="green">能使用jQuery等基本库操作dom，实现日常需要的交互效果,也能使用原生js实现各种功能,后面2个作品是由原生js实现，没有借助任何框架及类库.</span>\\<div></div>\\' +
            '<span>6. </span>\\<span class="green">了解过HTML5的canvas,借助canvas写过微信打飞机游戏</span>\\<div></div>\\' +
            '<span>7. </span>\\<span class="green">了解 Ajax 工作原理和实现方法,及对应的json的使用</span>\\<div></div>\\' +
            '<span>8. </span>\\<span class="green">使用过一段时间的CentOS 和 ubuntu,对linux各种常用命令有一定了解</span>\\<div></div>\\' +
            '<span>9. </span>\\<span class="green">了解过git的工作原理,并使用过一段时间</span>\\<div></div>\\' +
            '<span>10. </span>\\<span class="green">喜欢接触新事物，有一定的自学能力。</span>\\',
    ...
    let count = 2;
    //显示开机画面
    let xx = resume(str[0],10,text);
    xx.start();

    //显示登录界面
    setTimeout(()=> {
        let print = resume(str[1],10,text,true);
        print.start();
    },7000);
    ...
    enterBtn.addEventListener('touchend', () =>{
        ...
        if(count === 4){
            let print = resume(str[count],10,text);
            print.start();
        }else{
            let print = resume(str[count],10,text,true);
            print.start();
            count++;
        }
    },false)
